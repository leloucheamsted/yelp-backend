<?php
/**
 * Point d'entrée de l'API - Version SQLite
 * 
 * Endpoints:
 * GET /api-sqlite.php?action=search&term=restaurant&location=Paris&limit=10&offset=0
 * GET /api-sqlite.php?action=businesses&limit=10&offset=0
 */
...
try {
    ...
    if ($action === 'search') {
        ...
        // Sauvegarder en base
        $saved = $db->saveBusinesses($businesses);
        ...
        $businesses = $db->getBusinesses($limit, $offset, $search);
        $total = $db->countBusinesses($search);
        ...
        sendError('Action non supportée. Utilisez action=search ou action=businesses');
    }
